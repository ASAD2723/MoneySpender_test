...
const Carditem = ({
    product , basket , setBasket , total , money
}) => {
    ...
    const removeItem=()=>{
        ...
        const basketWithoutCurrentItem = basket.filter((item)=>item.id!==product.id);
        currentBasket.amount -= 1;
        if(currentBasket.amount === 0){
            setBasket([...basketWithoutCurrentItem])
        }
        else{
            setBasket([...basketWithoutCurrentItem , currentBasket])
        }
    }
  return (
